...
<LoadingOverlay />
<div class="container-fluid d-flex flex-column p-0" style="height: {dashboardHeight}vh; overflow-y: auto;">
  <!-- Navbar -->
  ...
  <div class="row flex-grow-1 m-1">
    ...
    <div class="bg-secondary p-0" on:mousedown={startVerticalResizing} style="width: 3px; cursor: col-resize;"></div>
  ...
  {#if !showSSH}
    <button class="btn btn-secondary custom-rounded-button me-auto ms-auto" on:click={toggleShowSSH}>
      {#if !sshInitialised}
...
{#if sshInitialised}
  <div class="row flex-grow-1 mx-0 bg-black {!showSSH ? 'visually-hidden' : ''}" style="height: {sshHeight}vh">
    ...
    <div class="col mx-1 px-0" style="height: calc(100% - 5px)">
